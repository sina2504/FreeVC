...
# Import the refactored functions from convert.py
...
async def convert_voices(batch_request: BatchVoiceConversionRequest):
    ...
    for idx, sample in enumerate(batch_request.samples):
        try:
            ...
            # Use a title to identify the conversion (this can be customized)
            title = f"sample_{idx}"
            
            # Convert the audio entirely in-memory using the refactored function.
